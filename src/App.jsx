...
// ─── DOTTED CIRCLE ────────────────────────────────────────────────────────────
const DottedCircle = memo(function DottedCircle({ radius, rotation = [0, 0, 0], color, segments = 64 }) {
  const geometry = useMemo(() => {
    const pts  = []
...
// ─── DOTTED ARC ───────────────────────────────────────────────────────────────
const DottedArc = memo(function DottedArc({ fromVec, toVec, radius, color, segments = 44 }) {
  const geometry = useMemo(() => {
...
export default function App() {
  ...
  const handleSystemChange = useCallback((id) => { setActiveSystemId(id); setNavOpen(false) }, [])
    ...
    return (
      <div className="flex flex-col h-screen bg-[#06091a] text-[#e2e8f0] overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 h-[52px] shrink-0 bg-[#030611] border-b border-white/[0.06]">
          <button onClick={() => setNavOpen(true)} className="bg-transparent border-0 text-[#7a90b0] cursor-pointer text-xl p-1 leading-none">☰</button>
    ...
    <div className="flex h-screen bg-[#06091a] text-[#e2e8f0] overflow-hidden">
      ...
      <div className="w-[210px] bg-[#030611] border-r border-white/[0.05] flex flex-col shrink-0">
        <div className="px-5 pt-6 pb-5 border-b border-white/[0.05]">
          ...
          <div className="text-[17px] font-bold text-[#c8d8ee] leading-tight">Coordinate<br />Systems</div>
        </div>
